...
for pr in prs:
    ...
    if not head.startswith('feat/impl/governance_'):
        continue
    num = pr.get('number')
    module = head.split('/')[-1]
    entry = {'pr': num, 'branch': head, 'module': module}
    testpath = os.path.join(ROOT, 'agent', 'governance', 'tests', f'test_{module}.py')
...
out_json = os.path.join(ROOT, 'implementation', 'impl_starts', 'GOV_IMPL_PROGRESS.json')
with open(out_json, 'w', encoding='utf-8') as f:
    json.dump(report, f, ensure_ascii=False, indent=2)

# also write a human-readable summary to Obsidian
lines = ['# Governance PR Auto-Merge Report']
for e in report:
    lines.append(f"- PR #{e['pr']} ({e['branch']}): module={e['module']}, pytest_rc={e.get('pytest_rc')}, coverage={e.get('coverage_line_rate')}, merged={e.get('merge')}")
summary_path = '/Users/huchufan/Documents/obsidian/obsidian_mac/INBOX_收集箱/Hermes/GOV_IMPL_PROGRESS.md'
