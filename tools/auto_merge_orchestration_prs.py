...
for pr in prs:
    ...
    if not head.startswith('feat/impl/orchestration_'):
        continue
    num = pr.get('number')
    module = head.split('/')[-1]
    entry = {'pr': num, 'branch': head, 'module': module}
    testpath = os.path.join(ROOT, 'agent', 'orchestration', 'tests', f'test_{module}.py')
...
out_json = os.path.join(ROOT, 'implementation', 'impl_starts', 'ORCH_IMPL_PROGRESS.json')
with open(out_json, 'w', encoding='utf-8') as f:
    json.dump(report, f, ensure_ascii=False, indent=2)

# human-readable summary
lines = ['# Orchestration PR Auto-Merge Report']
for e in report:
...
summary_path = '/Users/huchufan/Documents/obsidian/obsidian_mac/INBOX_收集箱/Hermes/ORCH_IMPL_PROGRESS.md'
