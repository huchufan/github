...
def gh_pr_list():
    p = subprocess.run(['gh','pr','list','--state','open','--json','number,headRefName,title','-L','300'], capture_output=True, text=True)
    if p.returncode != 0:
        return []
    try:
...
for pr in prs:
    ...
    if not head.startswith('feat/impl/multiagent_'):
        continue
    num = pr.get('number')
    module = head.split('/')[-1]
    entry = {'pr': num, 'branch': head, 'module': module}
    testpath = os.path.join(ROOT, 'agent', 'multiagent', 'tests', f'test_{module}.py')
    ...
    if lr >= 0.85:
        # ensure PR number exists; if not, try to find by head
        if num is None:
            opens = gh_pr_list()
            found = [o for o in opens if o.get('headRefName','')==head]
            if found:
                num = found[0].get('number')
                entry['pr'] = num
        if num is not None:
            ...
            entry['merge'] = False
            entry['reason'] = 'pr not found'
    else:
        entry['merge'] = False
        entry['reason'] = 'coverage below threshold'
    report.append(entry)

out_json = os.path.join(ROOT, 'implementation', 'impl_starts', 'MULTIAGENT_IMPL_PROGRESS.json')
with open(out_json, 'w', encoding='utf-8') as f:
    json.dump(report, f, ensure_ascii=False, indent=2)

lines = ['# Multiagent PR Auto-Merge Report']
for e in report:
    lines.append(f"- PR #{e.get('pr')} ({e.get('branch')}): module={e.get('module')}, pytest_rc={e.get('pytest_rc')}, coverage={e.get('coverage_line_rate')}, merged={e.get('merge')}, note={e.get('reason','')}")
summary_path = '/Users/huchufan/Documents/obsidian/obsidian_mac/INBOX_收集箱/Hermes/MULTIAGENT_IMPL_PROGRESS.md'
