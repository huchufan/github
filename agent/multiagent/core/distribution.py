...
class LoadBalancer:
    ...
    def analyze_load_distribution(self, agents):
        ...
        for a in agents:
            ...
            if cur >= avail:
                overloaded.append(
                    getattr(a, "agent_id", None) or getattr(a, "id", None)
                )
...
class TaskDistributionManager:
    ...
    async def distribute_task(self, task):
        ...
        agents = (
            await self.lifecycle_manager.get_all_agents()
            if self.lifecycle_manager
            else []
        )
        for a in agents:
            if task.required_skills and any(
                s in (a.capabilities or []) for s in task.required_skills
            ):
                # assign
                a.task_count += 1
                return a.agent_id
