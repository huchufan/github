...
def bootstrap_agents(registry):
    ...
    if missing_profiles:
        logger.warning("Agent registry bootstrap: %d/%d profiles missing or unreadable", len(missing_profiles), total)
    ...
    return {"total": total, "missing_count": len(missing_profiles), "missing": missing_profiles}
