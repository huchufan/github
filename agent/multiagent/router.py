...
ROUTING_TABLE_PATH = Path("/Users/huchufan/Documents/obsidian/obsidian_mac/INBOX_收集箱/Hermes/agent_model_routing_table.yaml")
...
def _call_model_sim(model_key: str, timeout_ms: int, task: Dict[str, Any]) -> Dict[str, Any]:
...
def choose_model(agent_id: str, task: Dict[str, Any]) -> Dict[str, Any]:
    ...
    timeouts = defaults.get('timeouts_ms', {'T1':3000,'T2':8000,'T3':1500,'T0':2000})
    ...
    order = [entry.get('preferred'), entry.get('upgrade'), entry.get('downgrade'), entry.get('fallback')]
    # normalize
    ...
    if task.get('cost_budget') is not None and task.get('cost_budget') < low_cost_threshold and 'T3' in order:
...
def run_health_check_once() -> dict:
    ...
    for model_key in models.keys():
        timeout = timeouts.get(model_key, 2000)
        try:
            res = _call_model_sim(model_key, timeout, {'risk':'LOW', 'requires_deep_reasoning': False})
            ...
            summary[model_key] = {'availability': 0, 'latency_ms': None, 'confidence': None, 'status': 'error', 'error': str(e)}
...
if __name__ == '__main__':
    # smoke test
    task = {'risk':'LOW','cost_budget':0.02,'requires_deep_reasoning':False,'sensitivity_level':'PUBLIC'}
