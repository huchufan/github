...
def test_bump_version_and_quality_tier():
    ...
    skill = SkillDefinition(
        name="s1", description="desc", code="def f(): pass", version="0.1.0"
    )
...
def test_create_new_skill_registers():
    ...
    opp = SkillCreationOpportunity(
        skill_name="auto", description="auto skill", pattern="p"
    )
...
def test_improve_existing_skill_updates_registry():
    ...
    base = SkillDefinition(
        name="base", description="d", code="def f(): pass", version="0.1.0"
    )
