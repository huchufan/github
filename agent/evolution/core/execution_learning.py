...
from agent.core.types import (ExecutionInsight, ExecutionResult,
                              ExecutionTrace, FailureAnalysis, ImpactAnalysis,
                              ParameterConstraints, RecoveryPattern,
                              SuccessFactors, SuccessfulConfiguration)
...
class ExecutionLevelLearning:
    ...
    async def analyze_execution(
        self, execution_result: ExecutionResult, execution_trace: ExecutionTrace
    ) -> ExecutionInsight:
        """分析执行结果并提取洞察。"""
        ...
        if execution_result.status == "SUCCESS":
            ...
            causes = self.analyze_failure_causes(
                execution_result.error or "", execution_trace
            )
        ...
        insight.resource_efficiency = self.calculate_resource_efficiency(
    ...
    def extract_success_factors(
        self, trace: ExecutionTrace, result: ExecutionResult
    ) -> SuccessFactors:
    ...
    def analyze_failure_causes(
        self, error: str, trace: ExecutionTrace
    ) -> FailureAnalysis:
    ...
    def calculate_resource_efficiency(
        trace: ExecutionTrace, result: ExecutionResult
    ) -> float:
    ...
    async def learn_from_success(self, success_factors: SuccessFactors) -> None:
        ...
        for skill in config.skill_selection:
            self.confidence_model[skill] = min(
                1.0, self.confidence_model.get(skill, 0.5) + 0.1
            )
...
class DecisionOptimizer:
    ...
    async def optimize_decision_parameters(
        self, execution_insights: List[ExecutionInsight]
    ) -> Dict[str, float]:
        """基于执行洞察优化决策参数。"""
        ...
        for param_name, analysis in impact.items():
            optimal = await self.optimize_parameter_value(
                param_name, analysis, ParameterConstraints(0.0, 1.0)
            )
            optimized[param_name] = optimal
        return optimized

    @staticmethod
    def collect_decision_history(
        insights: List[ExecutionInsight],
    ) -> List[Dict[str, Any]]:
        return [
            {"decision_path": i.decision_path, "metrics": i.performance_metrics}
            for i in insights
        ]
    ...
    def analyze_parameter_impact(
        ...
        if not impact:
            impact["parallelism"] = ImpactAnalysis(
                param_name="parallelism", optimal_value=0.5
            )
        return impact

    async def optimize_parameter_value(
        self,
        param_name: str,
