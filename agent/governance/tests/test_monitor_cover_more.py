...
class Op:
    def __init__(self, actor_role=None, action=None, operation_status=None, involves_sensitive_data=False):
...
def test_detect_anomaly_and_compliance_violation():
    gm = GovernanceMonitor()
    op_fail = Op(actor_role='service', operation_status='FAILURE', involves_sensitive_data=True)
