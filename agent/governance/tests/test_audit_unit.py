...
from agent.core.types import Actor, ExecutionContext, OperationResult, Resource
...
def test_audit_analyzer_detect_privilege_escalation():
    ...
    resource = Resource(
        type="config", id="c1", owner="admin", classification="INTERNAL"
    )
    ctx = ExecutionContext(request_id="r2")
    fail_result = OperationResult(status="FAILURE", code=1, error="denied")
    # log a high risk op
    logger.log_operation(
        "policy:manage", actor, resource, "try manage", fail_result, ctx
    )
