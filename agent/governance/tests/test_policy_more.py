...
from agent.core.types import Actor, Resource, Operation, OperationResult, ExecutionContext
...
def test_rule_engine_and_logic_and_or():
    engine = RuleEngine()
    p = Policy(id='p1', name='test', conditions=[PolicyCondition(field='role', operator='eq', value='admin'), PolicyCondition(field='resource.type', operator='eq', value='service')], condition_logic='AND')
...
def test_policy_validator_detects_failed_condition_and_limit():
    ...
    policy = Policy(id='p-delete', name='del', applies_to=['delete'], conditions=[PolicyCondition(field='role', operator='eq', value='admin')], limits=[PolicyLimit(limit_type='requests', max_count=1)])
...
def test_record_usage_and_determine_action():
    ...
    policy = Policy(id='p2', name='high', applies_to=['update'], conditions=[PolicyCondition(field='role', operator='eq', value='admin')], violation_severity='HIGH')
    pv.register_policy(policy)
