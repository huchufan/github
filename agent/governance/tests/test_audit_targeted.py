...
from agent.core.types import (Actor, AuditRecord, ExecutionContext,
                              OperationResult, Resource)
from agent.governance.core.audit import (AnomalyReport, AuditAnalyzer,
                                         AuditLogger)
...
def test_audit_analyzer_privilege_escalation_and_failure_rate():
    ...
    for i in range(55):
        logger.log_operation(
            ...
            res_s,
            f"read {i}",
            OperationResult(status="SUCCESS", code=0),
            ctx,
        )
