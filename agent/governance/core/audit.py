...
class AuditLogger:
    ...
    def log_operation(
        ...
        success = (
            getattr(result, "status", "") == "SUCCESS" if result is not None else True
        )
        ...
        if not rec.success or classification in ("CONFIDENTIAL", "SECRET"):
            alert = AnomalyReport(
                type="AUDIT_ALERT",
                severity="HIGH",
                description=f"Alert for {rec.resource_type}",
            )
    ...
    def query(
        ...
        return self._log.query(
            actor=actor_id, resource_type=resource_type, success=success
        )
...
class AuditAnalyzer:
    ...
    def detect_privilege_escalation(
        ...
        for r in records:
            ...
            action = getattr(r, "operation_type", None) or getattr(
                r, "action_details", ""
            )
            role = getattr(r, "actor_role", None) or getattr(
                getattr(r, "details", {}), "get", lambda k, d=None: None
            )("actor_role")
            ...
            if is_policy and low_priv:
                anomalies.append(
                    AnomalyReport(
                        ...
                        severity="HIGH",
                        description=f"Potential privilege escalation by {actor} on action {action}",
                    )
                )
        return anomalies

    def detect_anomalies(
        self, time_window: Optional[Any] = None
    ) -> List[AnomalyReport]:
        ...
        conf = [
            ...
            or getattr(r, "involves_sensitive_data", False)
        ]
        actors = {}
        for r in conf:
            ...
            if actors[getattr(r, "actor_id", "")] > 50:
                anomalies.append(
                    AnomalyReport(
                        ...
                        severity="HIGH",
                        description=f'Actor {getattr(r, "actor_id", "")} accessed many sensitive resources',
                    )
                )
    ...
    def generate_compliance_report(
        self, start: Any, end: Any, policy_name: str
    ) -> ComplianceReport:
