...
from agent.core.types import (Anomaly, ExecutionMetrics, ExecutionPlan,
                              ExecutionState, Severity, SubTask, TaskResult)
...
class ExecutionMonitor:
    ...
    def identify_bottleneck_tasks(self, state: ExecutionState):
        ...
        return [
            t.task_id for t in state.tasks_completed if getattr(t, "duration", 0) > 30
        ]
    ...
    def monitor_execution(self, state: "ExecutionState") -> "ExecutionMetrics":
        ...
        metrics.average_task_duration = (
            self.calculate_avg_duration(state.tasks_completed)
            if getattr(state, "tasks_completed", None)
            else 0.0
        )
        metrics.bottleneck_tasks = self.identify_bottleneck_tasks(state)
        metrics.retry_rate = (
            self.calculate_retry_rate(state)
            if hasattr(self, "calculate_retry_rate")
            else 0.0
        )
        metrics.estimated_remaining_time = self.estimate_remaining_time(state)
        # success rate
        success_count = len(
            [
                t
                for t in getattr(state, "tasks_completed", [])
    ...
    def calculate_retry_rate(self, state: "ExecutionState") -> float:
        ...
        total_tasks = max(
            1,
            len(getattr(state, "tasks_completed", []))
            + len(getattr(state, "tasks_failed", [])),
        )
    ...
    def estimate_remaining_time(self, state: "ExecutionState") -> float:
        ...
        remaining_levels = max(
            0,
            len(getattr(state.plan, "execution_order", []))
            - len(getattr(state, "tasks_completed", [])),
        )
    ...
    def detect_anomalies(self, state: "ExecutionState"):
        ...
        for t in getattr(state, "tasks_in_progress", []):
            ...
            if (
                start
                and timeout
                and (ExecutionMonitor._now_seconds() - start.timestamp()) > timeout
            ):
                anomalies.append(
                    Anomaly(
                        ...
                        suggestion="Investigate task timeout",
                    )
                )
        # high failure rate
            ...
            if recent_failure_rate > 0.5:
                anomalies.append(
                    Anomaly(
                        ...
                        severity=Severity.HIGH.value,
                        suggestion="High recent failure rate",
                    )
                )
        return anomalies
...
class AdaptiveReplanner:
    ...
    def should_replan(
        self, execution_state: "ExecutionState", metrics: "ExecutionMetrics"
    ) -> bool:
        try:
            if metrics is None:
                return False
            ...
            plan_est = getattr(
                getattr(execution_state, "plan", None), "time_estimate", 0.0
            )
            if plan_est and getattr(metrics, "estimated_remaining_time", 0.0) > (
                plan_est * 2
            ):
                return True
        except Exception:
