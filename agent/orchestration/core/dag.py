...
class DAG:
    ...
    def topological_sort(self):
        # backward-compatible wrapper for tests expecting flat topo order
    ...
    def find_critical_path(self):
        ...
        def dfs(node, visited):
            best = [node]
            for e in [ed for ed in self.edges if ed.source == node]:
                if e.target in visited:
                    continue
                path = [node] + dfs(e.target, visited | {node})
                if len(path) > len(best):
                    best = path
            return best
        # run DFS from all roots
        roots = [n for n in self.nodes if all(e.target != n for e in self.edges)]
        best_overall = []
        for r in roots:
            p = dfs(r, set())
