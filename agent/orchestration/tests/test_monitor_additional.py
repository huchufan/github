...
from agent.core.types import ExecutionState, ExecutionPlan, SubTask, TaskResult
...
def test_identify_bottleneck_tasks_and_avg_duration():
    ...
    tr1 = TaskResult(task_id='t1', status='SUCCESS', result={}, duration=40.0, success=True)
    tr2 = TaskResult(task_id='t2', status='SUCCESS', result={}, duration=5.0, success=True)
...
def test_estimate_remaining_time_and_critical_path_progress():
    ...
    tr = TaskResult(task_id='t1', status='SUCCESS', result={}, duration=1.0, success=True)
...
def test_detect_anomalies_timeout_and_failure_rate():
    ...
    tr_fail = TaskResult(task_id='t_prev', status='FAIL', result={}, duration=1.0, success=False)
