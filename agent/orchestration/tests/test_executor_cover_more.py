...
from agent.orchestration.core.executor import ErrorHandlingStrategy, OrchestrationEngine
from agent.core.types import SubTask, ExecutionPlan, TaskResult, ExecutionState, OperationStatus


def test_select_recovery_strategy_fallback_allowed_when_retries_exhausted():
    strat = ErrorHandlingStrategy()
    # ensure strategy exists
    strat.strategies['execute_fallback'] = {"action": "FALLBACK", "retries": 0, "fallback_skill": "alt"}
    task = SubTask(id='t1', skill_name='orig', retry_policy='execute_fallback')
    # retry_count >= retries: should still return FALLBACK
    chosen = strat.select_recovery_strategy(task, error_category='GENERIC_ERROR', retry_count=0)
