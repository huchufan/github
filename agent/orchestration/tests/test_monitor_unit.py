...
def make_state_with_plan_and_completed():
    ...
    plan = ExecutionPlan(subtasks=[t1, t2], execution_order=[[t1],[t2]], time_estimate=10)
...
def test_detect_anomalies_timeout_and_failure_rate():
    ...
    for i in range(5):
        state.tasks_completed.append(TaskResult(task_id=f'c{i}', duration=1.0, success=(i<2)))
    anomalies = m.detect_anomalies(state)
    assert any(a.type == 'EXCESSIVE_TIMEOUT' or a.type=='HIGH_FAILURE_RATE' for a in anomalies)
