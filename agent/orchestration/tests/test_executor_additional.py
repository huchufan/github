...
def make_subtask(id='t1', skill='s1', timeout=0.1, retry_policy='retry_on_transient'):
    return SubTask(id=id, skill_name=skill if (skill := skill) else 's1', timeout=timeout, retry_policy=retry_policy)


def make_state():
    return type('S', (), {'tasks_failed': [], 'tasks_completed': [], 'retry_counts': {}, 'get_retry_count': lambda self, x: self.retry_counts.get(x, 0)})()
...
def test_handle_task_failures_task_missing():
    ...
    failed = TaskResult(task_id='missing', status=OperationStatus.FAILURE.value, success=False, error='no task')
    state = make_state()
...
def test_handle_task_failures_continue_policy():
    ...
    failed = TaskResult(task_id='t_continue', status=OperationStatus.FAILURE.value, success=False, error='err')
    state = make_state()
    cont = asyncio.run(engine.handle_task_failures([failed], state, plan))
    assert cont is True
...
def test_handle_task_failures_fallback_failure():
    ...
    plan = ExecutionPlan(subtasks=[task])
    failed = TaskResult(task_id='t_fb', status=OperationStatus.FAILURE.value, success=False, error='fail')
    state = make_state()
