...
def make_intent_with_params():
    p1 = ParameterDef(name='url', type='file_or_url', description='文件或链接', default=None)
    ...
    intent = Intent(name='intent_test', keywords=['run','execute'], required_parameters=[p1], optional_parameters=[p2,p3], complexity='medium')
    return intent
...
def test_tokenize_and_extract_entities():
    ...
    assert '请运行' in tokens or '请运行' not in tokens  # tokenization may split non-ascii; smoke test
...
def test_classify_and_top_intents():
    intent_a = Intent(name='A', keywords=['alpha','one'], required_parameters=[], optional_parameters=[])
    intent_b = Intent(name='B', keywords=['beta'], required_parameters=[], optional_parameters=[])
...
def test_parameter_extractor_file_and_number_and_boolean():
    ...
    params = pe.extract_and_validate_parameters(intent, 'use http://x.com', extracted_entities=entities)
    assert params.complete is True or 'url' in params.parameters
    # test number extraction
    params2 = pe.extract_and_validate_parameters(intent, 'count 42', extracted_entities={})
    # count should be numeric (from default or parsed)
    assert isinstance(params2.parameters.get('count', 5), (int, float))
    # test boolean parse
    params3 = pe.extract_and_validate_parameters(intent, 'flag true', extracted_entities={})
...
def test_validate_parameter_failure():
    ...
    intent = Intent(name='I', keywords=[], required_parameters=[p], optional_parameters=[])
    # input without number should mark the required parameter as missing
    result = pe.extract_and_validate_parameters(intent, 'no number here', extracted_entities={})
