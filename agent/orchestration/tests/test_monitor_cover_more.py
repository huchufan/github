...
def test_identify_bottleneck_threshold_and_none():
    ...
    tr = TaskResult(task_id='t1', status='SUCCESS', result={}, duration=10.0, success=True)
    state.tasks_completed = [tr]
    assert monitor.identify_bottleneck_tasks(state) == []
    # duration above threshold
    tr2 = TaskResult(task_id='t1', status='SUCCESS', result={}, duration=40.0, success=True)
...
def test_calculate_retry_rate_and_recent_failure_rate():
    ...
    ok = TaskResult(task_id='t1', status='SUCCESS', result={}, duration=1.0, success=True)
    fail = TaskResult(task_id='t2', status='FAIL', result={}, duration=1.0, success=False)
