...
def test_fallback_uses_alt_skill():
    engine = OrchestrationEngine(skill_registry={'orig': always_conn_err, 'alt': lambda p, c: {'ok': True}})
...
def test_timeout_classification_and_retry():
    ...
    plan = ExecutionPlan(subtasks=[task])
    failed = TaskResult(task_id='t_to', status=OperationStatus.FAILURE.value, success=False, error='timeout')
    state = type('S', (), {'tasks_failed': [], 'tasks_completed': [], 'retry_counts': {}, 'get_retry_count': lambda self, x: 0})()
    ok = asyncio.run(engine.handle_task_failures([failed], state, plan))
