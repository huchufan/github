...
class ImmediateContextMemory(MemoryLayer):
    ...
    def get_attention_context(self) -> Any:
        ...
        ac.focus_strength = (
            (1.0 / len(self.attention_stack)) if self.attention_stack else 0.0
        )
        return ac
...
class SessionMemory(MemoryLayer):
    ...
    def store_session_context(self, session_id: str, ctx: Any) -> None:
        ...
        data = {
            ...
            "user_preferences": getattr(ctx, "user_preferences", None)
            or {
...
class EpisodicMemory(MemoryLayer):
    ...
    def extract_learned_patterns(
        ...
        for i in range(len(seq) - 1):
            ...
            ev2 = seq[i + 1]["event"]
            if (
                getattr(ev1, "success", None) is False
                and getattr(ev2, "success", None) is True
            ):
                p = type("P", (), {})()
        ...
        if not patterns and seq:
            ...
            has_failure = any(
                getattr(item["event"], "success", None) is False for item in seq
            )
