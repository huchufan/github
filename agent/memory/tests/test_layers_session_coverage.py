...
from agent.memory.core.layers import SessionMemory, EpisodicMemory, SemanticMemory, ArchiveMemory
from agent.core.types import SessionRecord, EventRecord, KnowledgeRecord
...
def test_search_limit_break():
    sm = SessionMemory(ttl=timedelta(days=1))
    for i in range(10):
        sm.store(f's{i}', SessionRecord(session_id=f's{i}', user_id='u', summary=f'msg {i}'))
