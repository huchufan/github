...
from agent.memory.core.layers import (ArchiveMemory, KnowledgeItem,
                                      SemanticMemory, SessionRecord)


def test_semantic_store_and_search_and_graph():
    sm = SemanticMemory(dim=16)
    item = KnowledgeItem(
        title="T", content="some content about AI", category="ai", concepts=["ai"]
    )
