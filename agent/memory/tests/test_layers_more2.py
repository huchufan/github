...
from agent.core.types import (ConversationTurn, KnowledgeItem, SessionRecord,
                              SystemEvent)
from agent.memory.core.layers import (ArchiveMemory, EpisodicMemory,
                                      ImmediateContextMemory, LRUCache,
...
def test_episdodic_record_and_patterns():
    ...
    assert any(
        p.pattern_type in ("success_sequence", "failure_recovery") for p in patterns
    )
