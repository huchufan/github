...
from agent.core.types import EventRecord, KnowledgeItem, SessionRecord
from agent.memory.core.layers import (ArchiveMemory, EpisodicMemory, LRUCache,
...
def test_archive_checksum_mismatch_raises():
    am = ArchiveMemory()
    s = SessionRecord(
        session_id="sa2", user_id="uu", created_at=datetime.now(timezone.utc)
    )
