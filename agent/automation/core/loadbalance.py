...
class LoadBalancingManager:
    ...
    async def distribute_workflow_execution(self, workflow: Workflow, available_workers: List[Worker]) -> Worker:
        """分配工作流到最合适的工作进程。"""
        if not available_workers:
            raise NoAvailableWorkersError()
        scored = [(w, self.evaluate_worker_suitability(w, workflow)) for w in available_workers]
    ...
    async def handle_worker_failure(self, failed_worker: Worker, workflow: Workflow) -> Worker:
        """处理工作进程失败并转移到备用进程。"""
